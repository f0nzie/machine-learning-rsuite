# Classification algorithm comparison. *Orange Juice* dataset
...
### Algorithms used
1. Random Forest
2. Multivariate Adaptive Regresion Splines
3. AdaBoost
4. XGBoost DART
5. SVM
...
1. Compare models
    - summary
    - Box plot of ROC and Sensitivity
1. Ensembling
    - Training 
        - RF
        - AdaBoost
...
## Initial Setup – load the package and dataset
...
### How to split the dataset into training and validation?
...
```{r featurePlot-density_training}
featurePlot(x = trainData[, 1:18], 
            y = trainData$Purchase, 
            plot = "density",
            strip=strip.custom(par.strip.text=list(cex=.7)),
...
```{r models-ensemble-run-takes_a_while}
...
```{r boxplot_comparison_enssemble}
